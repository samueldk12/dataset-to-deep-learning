...
export const ImageStrip: React.FC<ImageStripProps> = ({
  ...
  const [filter, setFilter] = React.useState<'all' | 'annotated' | 'empty'>('all');
  ...
  return (
    <div className="h-28 bg-slate-900 border-t border-slate-800 flex flex-col justify-between px-3 py-2 select-none z-10 shrink-0">
      {/* Top row: Counter, Filter, Navigation */}
      <div className="flex items-center justify-between text-xs text-slate-400 mb-1.5">
        <div className="flex items-center gap-2">
          <span className="font-semibold text-slate-200 flex items-center gap-1.5">
            <ImageIcon className="w-3.5 h-3.5 text-blue-400" />
            Galeria do Dataset ({images.length} imagens)
          </span>
      ...
      >
        ...
        <button
          onClick={onAddImages}
          className="group flex flex-col items-center justify-center min-w-[90px] h-[64px] rounded-xl border border-dashed border-slate-700 hover:border-blue-500 bg-slate-950/40 hover:bg-blue-600/10 text-slate-400 hover:text-blue-400 transition-all shrink-0 cursor-pointer"
        ...
        {filteredImages.map((img) => {
          ...
          return (
            <div
              ...
              className={`group relative min-w-[96px] max-w-[96px] h-[64px] rounded-xl overflow-hidden cursor-pointer transition-all border shrink-0 ${
                isSelected
